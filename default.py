...
def Dashboard():
    ...
    #disk usage
    cmd_disk = "/home/www-data/waf2py_community/scripts/getrootdevice.sh"
...
def CreateNewApp():
    ...
    try:
        if request.vars['app_url'] != '' and len(request.vars['app_url']) < 45 and  request.vars['name'] != '' and len(request.vars['name']) < 45:
            ...
            if any(c in AppName for c in "\"/'\;,=%#$*()[]?Â¿Â¡{}:!|&<>Â¨~Â°^ "):
            ...
            elif any(c in request.vars['name'] for c in "\"/'\;,=%#$*()[]?Â¿Â¡{}:!|&<>Â¨~Â°^ "):
...
def EngxEdit():
    ...
    #hacer filtro en caso de que vengan los datos en blanco (si se llama la funcion directamente arrojarÃ¡ error)
    a = stuffs.Filtro()
    b = a.CheckStr(request.args[0])
...
def BackendIps():
    ...
    if b == 'YES':
        ...
        try:
            ...
            check_list = []
            if any(c in IPS_http for c in "\"';,%#$*=()[]{}?Â¿Â¡!|&<>Â¨~Â°-^ "):
                check_list.append('NO')
            if any(c in IPS_https for c in "\"';,%#$*=()[]{}?Â¿Â¡!|&<>Â¨~Â°-^ "):
                check_list.append('NO')
            else:
